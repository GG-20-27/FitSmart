...
function parseRoutine(text: string): { totalTime: string; steps: { time: string; action: string; sub?: string }[]; why: string[] } {
  ...
  for (const line of lines) {
    ...
    // Coaching sub-line after a step
    if (steps.length > 0) {
      steps[steps.length - 1].sub = line;
    }
  }
...
export default function FitScoreScreen() {
  ...
  const handleActivatePlan = async (pillar: string, bedtime?: string, wakeTime?: string) => {
    ...
    try {
      ...
      // Re-fetch full status so any new pendingPlan (e.g. recovery after nutrition activated) appears
      try {
  ...
  const handlePlanHabitToggle = async (habitKey: string, current: boolean) => {
    const next = !current;
    setPlanHabits(prev => prev.map(h => h.habit_key === habitKey ? { ...h, checked: next } : h));
    const todayStr = formatDate(new Date());
    togglePlanHabit(todayStr, habitKey, next).catch(() => {});
  };
    ...
    >
              ...
              {planHabits.length > 0 && (
                <>
                  ...
                  <View style={styles.planHabitsHeader}>
                    ...
                    <Text style={styles.planHabitsLabel}>Plan Habits</Text>
                  </View>
                  {planHabits.map(h => (
                    <TouchableOpacity
                      key={h.habit_key}
                      style={styles.habitRow}
                      onPress={() => handlePlanHabitToggle(h.habit_key, h.checked)}
                      activeOpacity={0.7}
                    >
                      <View style={[styles.habitCheckbox, h.checked && styles.habitCheckboxDone]}>
                        {h.checked && <Ionicons name="checkmark" size={13} color="#FFFFFF" />}
                      </View>
                      <Text style={[styles.habitText, h.checked && styles.habitTextDone]}>{h.label}</Text>
                    </TouchableOpacity>
                  ))}
                </>
              )}
      ...
      {showFitScoreResult && fitScoreResult && (
        ...
        >
          ...
          {/* Improvement Plans — each plan gets its own standalone card */}
          {improvementPlanStatus && (
            <>
              {/* Active plans — one card each */}
              {(improvementPlanStatus.activePlans ?? (improvementPlanStatus.activePlan ? [improvementPlanStatus.activePlan] : [])).map((plan) => {
                ...
                return (
                  <View key={plan.id} style={styles.improvementPlanSection}>
              ...
              {/* Pending plans — one standalone card per pillar */}
              {(improvementPlanStatus.pendingPlans ?? (improvementPlanStatus.pendingPlan ? [improvementPlanStatus.pendingPlan] : [])).map((pending) => {
                ...
                return (
                  <View key={pending.pillar} style={styles.improvementPlanSection}>
                    <View style={styles.planCardHeader}>
                      <View style={styles.planCardIconWrap}>
                        <Ionicons name="shield-checkmark-outline" size={15} color={colors.accent} />
                      </View>
                    ...
                    <View style={styles.improvementPlanCard}>
                      {pending.unlocked ? (
                        ...
                        <>
                          <Text style={styles.planActiveName}>{`${label} is your weak pillar`}</Text>
                          <Text style={styles.planActiveMeta}>Lowest pillar this week</Text>
                          <View style={styles.planDotsRow}>
                            {[0,1,2,3,4].map(i => (
                              <View key={i} style={[styles.planDot, i < pending.weaknessCount && styles.planDotFilled]} />
                            ))}
                            <Text style={styles.planDotsLabel}>{`${pending.weaknessCount}/5 — plan unlocks at 5`}</Text>
                          </View>
                        </>
                      )}
                    </View>
                      ...
                      <>
                        ...
                        <View style={styles.trainingDirectionCard}>
                          ...
                          <TouchableOpacity
                            ...
                            activeOpacity={0.7}
                            onPress={() => {
                              // Pre-select goal based on direction
                                ...
                                <View key={meal} style={styles.fitCookTimeRow}>
                                  ...
                                  <View style={styles.fitCookTimeFields}>
                                    <TextInput
                                      ...
                                      onBlur={() => {
                                        const v = fitCookTimes[meal];
                                        if (v) setFitCookTimes(t => ({ ...t, [meal]: normalizeTime(v) }));
                                      }}
...
const styles = StyleSheet.create({
  ...
  // Plan habits sub-block inside Daily Habits
  planHabitsDivider: {
    height: 1,
    backgroundColor: colors.surfaceMute,
    marginVertical: spacing.sm,
